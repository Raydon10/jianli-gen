...
.tippy-box[data-animation=fade][data-state=hidden] {
  ...
  var autoTokenizePluginKey = new PluginKey("jianli-auto-tokenize");
  ...
  function renderSlashSuggestionMenu(root, props, items, activeIndex, onSelect) {
    ...
    const content = items.length ? items.map((item, index) => {
      ...
      return `
          ...
          `;
    }).join("") : `<div class="slash-empty">\u6CA1\u6709\u5339\u914D\u9879</div>`;
  ...
  var AutoTokenizeExtension = Extension.create({
    name: "auto-tokenize",
    addOptions() {
      return {
        getFields: () => []
  ...
  var SlashSuggestionExtension = Extension.create({
    ...
    addProseMirrorPlugins() {
      ...
      const updateMenu = (props) => {
        ...
        const items = (getFields() || []).filter((field) => field.key && (!normalizedQuery || field.key.toLowerCase().includes(normalizedQuery))).slice(0, 8);
        activeItems = items;
        if (!items.length) {
          destroyMenu();
          return;
      ...
      return [
        Suggestion({
          ...
          items: ({ query }) => {
            ...
            return (getFields() || []).filter((field) => field.key && (!normalizedQuery || field.key.toLowerCase().includes(normalizedQuery))).slice(0, 8);
          },
          render: () => ({
  ...
  function createJianliEditor(root, options = {}) {
    ...
    const editor = new Editor({
      ...
      onCreate: ({ editor: editor2 }) => {
        suppressChange = true;
        editor2.commands.setContent(buildDoc(options.initialText || "", getFields()));
      ...
      onUpdate: ({ editor: editor2 }) => {
        ...
        currentSourceText = serializeDoc(editor2.getJSON());
        onChange(currentSourceText, editor2);
      },
